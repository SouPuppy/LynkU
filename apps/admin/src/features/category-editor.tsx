...
import type { CategoryChange, CategoryCreation, CategoryChangeReceipt } from '@lynku/contracts'
import { updateCategory, createCategory, AdminRequestError, type AdminCategory } from '../lib/admin-client'
...
export function CategoryEditor({ category: existing, onClose, onSaved }: { category: AdminCategory | null; onClose: () => void; onSaved: () => void }) {
  const category = existing ?? { _id: 'new', name: '', description: '', sort_order: 10, status: 'active' as const, post_count: 0, managementRevision: 0 }
  const [draft, setDraft] = useState(category)
  const [reason, setReason] = useState('')
  const [request, setRequest] = useState<CategoryChange | CategoryCreation | null>(null)
  ...
  async function submit(event: FormEvent) {
    ...
    const operation = failure?.uncertain && request ? request : existing ? { requestId: crypto.randomUUID(), category: draft, reason }
      : { requestId: crypto.randomUUID(), reason, name: draft.name, description: draft.description, sort_order: draft.sort_order, status: draft.status }
    inFlight.current = true; setRequest(operation); setPending(true); setFailure(null)
    try {
      const value = 'category' in operation ? await updateCategory(operation) : await createCategory(operation)
  ...
  return <Sheet open onOpenChange={open => { if (!open && !pending && !failure?.uncertain) onClose() }}>
    <SheetContent className="w-full overflow-y-auto sm:max-w-xl" showCloseButton={!pending && !failure?.uncertain}>
      <SheetHeader><SheetTitle>{existing ? '编辑分类' : '新增分类'}</SheetTitle><SheetDescription>{existing ? `修改 ${category.name} · 当前版本 ${category.managementRevision}` : '创建新的帖子分类，最多100个（含停用项）'}。保存后影响分类展示和发帖可选项。</SheetDescription></SheetHeader>
      <form onSubmit={submit} className="grid gap-5 px-4 pb-6">
        ...
        {failure && <p role="alert" className="text-sm text-destructive">{failure.code === 'CONFLICT' ? existing ? '该分类已被修改。请返回列表刷新后重新编辑。' : '名称已存在或分类数量达到上限，请返回列表核对。' : failure.message}</p>}
