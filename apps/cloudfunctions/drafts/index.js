...
const { saveUserDraft, listUserDrafts, deleteUserDraft, DraftFailure } = require('@lucky/server')
      ...
      return db.runTransaction(transaction => operation({
        ...
        async putDraft(id, row) {
          ...
          await transaction.collection('drafts').doc(id).set({ data: {
            ...data, created_at: new Date(data.created_at), updated_at: new Date(data.updated_at),
          } })
        },
