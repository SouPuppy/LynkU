...
- 内容审核接口是可替换的策略。保留现有敏感词行为作为起点；缺少真实服务验证时不得宣称已满足上线审核要求。
...
- 匿名通道以发现对方的具体来源（帖子或评论）、发起者和来源作者共同确定。同一人从同一来源再次进入复用已有通道；从另一帖子、另一评论或同帖下不同评论进入则分别建立通道，即使真实作者相同。不同访客从同一来源进入也各自隔离。会话列表、缓存、消息、未读与已读操作均按通道区分，不得按真实对端合并或提示跨来源为同一人。
