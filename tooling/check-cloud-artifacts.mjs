...
function validateArtifact(name, artifact) {
  ...
  assert.deepEqual(builtPackage, {
    ...
    dependencies: { 'wx-server-sdk': sourcePackage.dependencies['wx-server-sdk'] },
    overrides: runtimePackage.overrides,
  }, `${name}: artifact package does not match source runtime dependencies`)
  assert.match(builtPackage.dependencies['wx-server-sdk'], /^\d+\.\d+\.\d+$/, `${name}: SDK version must be exact`)
  ...
  assert.ok(metadata.external.includes('wx-server-sdk'), `${name}: SDK external declaration is missing`)
  for (const dependency of metadata.external) {
    assert.ok(isBuiltin(dependency) || dependency === 'wx-server-sdk', `${name}: undeclared external dependency ${dependency}`)
  }
}

// Run outside the repository and prohibit resolving dependencies from its node_modules.
const directory = fs.mkdtempSync(path.join(os.tmpdir(), 'lucky-cloud-artifacts-'))
try {
  ...
  if (path.dirname(directory) !== path.resolve(os.tmpdir()) || !path.basename(directory).startsWith('lucky-cloud-artifacts-')) throw new Error('Unexpected temporary directory')
