...
const checks = [
  ...
  ['listOperationTasks', parseOperationPage, { kind: 'notifications' }],
  ['listOperationTasks', parseOperationPage, { kind: 'profiles' }],
...
for (const [action, validate, input = {}] of checks) {
  try {
    const call = spawnSync(process.execPath, [path.join(root, 'node_modules/@cloudbase/cli/bin/tcb'), 'fn', 'invoke', 'admin', '-e', project.cloudEnvironment, '-d', JSON.stringify({ ...input, action }), '--json'], { cwd: root, encoding: 'utf8' })
    ...
    report.checks.push({ action, input, passed: true, requestId: invocation.FunctionRequestId })
