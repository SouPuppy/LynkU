...
import { parseAdminPostPage, parseAdminUserPage, parseManagedCategoryList, parseAdminMembers } from '@lynku/contracts'
const checks = [
  ...
  ['listAudit', value => { if (!Array.isArray(value.events)) throw Error('Invalid audit response'); for (const row of value.events) { if (!row.id || !row.action || !row.target || !Number.isFinite(Date.parse(row.at))) throw Error('Invalid audit record') } }],
  ['listMembers', value => parseAdminMembers(value.members)],
]
const report = { environment: project.cloudEnvironment, at: new Date().toISOString(), privilegedCli: true, browserVerified: false, checks: [] }
mkdirSync(path.join(root, 'dist'), { recursive: true })
for (const [action, validate] of checks) {
  try {
    const call = spawnSync(process.execPath, [path.join(root, 'node_modules/@cloudbase/cli/bin/tcb'), 'fn', 'invoke', 'admin', '-d', JSON.stringify({ action }), '--json'], { cwd: root, encoding: 'utf8' })
