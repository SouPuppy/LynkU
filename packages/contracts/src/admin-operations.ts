...
export interface OperationTask { id: string; kind: OperationKind; status: OperationStatus; createdAt: string; attempts: number; nextAttemptAt: number | null; leaseUntil: number | null; lastAttemptAt: number | null; deliveredAt: string | null; lastError: 'DELIVERY_FAILED' | 'UNCLASSIFIED_ERROR' | null }
