export type AnonymousChatTarget =
  ...
  | { anonymous: true; type: 'post' | 'comment' | 'user'; id: string; initiation_id: string; initiator_visibility: 'anonymous' | 'real'; expected_target_visibility?: 'anonymous' | 'real' }
...
export function parseAnonymousChatTarget(value: unknown): AnonymousChatTarget {
  ...
  if (Object.keys(input).some(key => !['anonymous', 'type', 'id', 'initiation_id', 'initiator_visibility', 'expected_target_visibility'].includes(key))
  ...
  if (input.expected_target_visibility !== undefined && input.expected_target_visibility !== 'anonymous' && input.expected_target_visibility !== 'real') throw Error('Invalid expected visibility')
  return { anonymous: true, type: input.type, id: input.id, initiation_id: input.initiation_id, initiator_visibility: input.initiator_visibility,
    ...(input.expected_target_visibility === undefined ? {} : { expected_target_visibility: input.expected_target_visibility }) }
}
