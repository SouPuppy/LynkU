export type AnonymousChatTarget =
  ...
  | { anonymous: true; type: 'post' | 'comment' | 'user'; id: string; initiation_id: string }
...
export function parseAnonymousChatTarget(value: unknown): AnonymousChatTarget {
  ...
  if (Object.keys(input).some(key => !['anonymous', 'type', 'id', 'initiation_id'].includes(key))
  ...
  return { anonymous: true, type: input.type, id: input.id, initiation_id: input.initiation_id }
}
